        ...
        while self._asyncio_tasks:
            ...
            for task in done_tasks:
                ...
                try:
                    ...
                    await task
                    # self.logger.warning("task %s finished", task)
                except asyncio.CancelledError:
                    # suppressing the exception here is *ok* because the
                    # `CancelledError` is not important to the task lifecycle.
                    pass
                    # self.logger.warning("task %s cancelled", task)
                finally:
                    # self.logger.warning("task %s discarded", task)
        ...
        async with self._run_lock:
            ...
            async with cleanup_tasks(handle_cancelled_task):
                ...
                try:
                    ...
                    self.logger.warning("self._wait_all_tasks_done()")
                    await self._wait_all_tasks_done()
                except KeyboardInterrupt:
                    self.logger.exception("KeyboardInterrupt in %s._wait_all_tasks_done", self)
                    raise
                except BaseException:
                    self.logger.exception("Error waiting for self._wait_all_tasks_done")
                    raise
                finally:
                    self.logger.warning(
                        "self._wait_all_tasks_done finished; cleanup_tasks will kick in")

        self.logger.warning("%s: got to the end of run()", self)
        self._finished.set()
    ...
    try:
        async with cleanup_tasks(task):
            ...
            try:
                ...
                # XXX: The above is not correct; when the control is returned to us with a
                # KeyboardInterrupt, the task running manager.run() is still pending. We probably
                # don't want to handle
                yield manager
            except KeyboardInterrupt:
                manager.logger.exception("KeyboardInterrupt in background_run1: %s", service)
                raise
            finally:
                manager.logger.warning("Waiting for %s.manager.stop()", service)
                try:
                    # await asyncio.sleep(0.1)
                    await manager.stop()
                except KeyboardInterrupt as ee:
                    manager.logger.warning("%s.manager.stop() errored: %s", service, repr(ee))
                else:
                    manager.logger.warning("%s.manager.stop() finished", service)
    finally:
        ...
        manager.logger.warning("background_asyncio_service() returning cleanly")
